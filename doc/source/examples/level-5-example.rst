...
.. code-block:: bash
    ...
    [12/16] project_keywords (diffraction, PDF, X-ray, neutron): knights, castle,
    Monty, Python,→
...
The questions are designed to be somewhat self-describing, but what they mean and how they are used is described in detail in the ``scikit-package`` documentation.
...
After setting up the repository structure, **Sir Lancelot** adds code to the empty package by creating files in the ``./src/montpy`` directory. Any unit tests he adds in the tests directory. Previously written files can also be copied over from wherever they were on his hard drive, as in Example 1. Below we show the src and tests part of the directory tree after **Sir Lancelot** did this:
...
Once created, the package can be put under Git control and pushed to a repository with the name **montypy** that **Sir Lancelot** creates at GitHub, following the approach in Section 4.1.4 (FIXME). In the example, **Sir Lancelot** chose to create the new GitHub repository called **montypy**, under the **kot-roundtable** GitHub organization. The step-by-step tutorial for doing this is provided in the ``scikit-package`` documentation (FIXME). In this example the code could then be found at https://github.com/kot-roundtable/montypy.
...
**Sir Robin** then clones this fork to his local computer with:
...
On **Sir Robin**'s local computer he has a clone of the repository that is linked to his fork, but it doesn't automatically know that the fork is linked to a repository upstream of the fork in the **kot-roundtable** org. **Sir Robin** then types the following to tell his local
repository about the upstream repository:

.. code-block:: bash

    $ git remote add upstream https://github.com/kot-roundtable/montypy.git

**Sir Robin** can now keep his local repository synchronized with all changes that are merged into the upstream repository by typing:
...
In **Sir Robin**'s branch he starts by defining a ``test_bucket()`` test function in test ``utils.py``, and defining an empty function ``def bucket()``: in the ``utils.py`` module. **Sir Robin** then stages and commits the changes. The commands for the staging and committing he uses are:
...
We recommend that contributors share code with maintainers as early as possible in the process, allowing for rapid, early feedback. In the example, **Sir Robin** does this by creating a “Pull request”, or PR, on the upstream repository in order to solicit the feedback. To accomplish this **Sir Robin** first must push the new branch with the local changes to his linked repository on GitHub using:
...
When **Sir Robin** created the PR, several separate GitHub workflows were automatically triggered. These workflows are controlled by workflow files, located in the .github/workflows directory, that were created when **Sir Lancelot** started a new Level 5 project using ``scikit-package``. Here are the workflows that both **Sir Robin** and **Sir Lancelot** would see:
  ...
  #. The second CI workflow runs pre-commit to check the code quality similar to when pre-commit is run locally (described in Section 2 FIXME). To ensure that this CI test passes, get in the habit of running pre-commit locally before committing and installing pre-commit as a commit hook (Section 4.1.5) #FIXME
...
Semantic versioning involves three numbers separated by two periods, where the three numbers indicate MAJOR, MINOR and PATCH release numbers. In this example, it is an initial release so **Sir Lancelot** chooses the lowest non-patch release number, ``0.1.0``. I.e., he types:
...
We recommend to also make packages available on conda-forge which can host not just python and has powerful methods for checking the dependency tree of all packages. For hints for how to do this please see the ``scikit-package`` documentation.
